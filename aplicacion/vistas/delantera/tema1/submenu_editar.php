<div class="row">
  <div class="col-md-6 col-md-offset-3">
    ...
    ?>
    <form class="form-horizontal" role="form" action="<?= base_url('submenus/editar/'.$idsub); ?>" method="post">
      <fieldset><legend>Editar Submenú</legend>
      <input type="hidden" id="idsub" name="idsub" value="<?php echo obt_valor('idsub',$idsub) ?>" >
      <div class="form-group <?= $e_idmen; ?>">
        <label class="col-lg-4 control-label" for="idmen">Menú</label>
        <div class="col-lg-5">
        <?= agr_selector('idmen',$menus,'idmen',$idmen,'class="form-control"'); ?>
        </div>
      </div>
      <div class="form-group <?= $e_etiqueta; ?>">
        <label class="col-lg-4 control-label" for="submenu_etiqueta">Etiqueta</label>
        <div class="col-lg-8">
          <input type="text" class="form-control" name="submenu_etiqueta" id="submenu_etiqueta" value="<?= obt_valor('submenu_etiqueta',$submenu_etiqueta); ?>" placeholder="Etiqueta">
        </div>
      </div>
      <div class="form-group <?= $e_url; ?>">
        <label class="col-lg-4 control-label" for="submenu_url">Url</label>
        <div class="col-lg-8">
          <input type="text" class="form-control" name="submenu_url" id="submenu_url" value="<?= obt_valor('submenu_url',$submenu_url); ?>" placeholder="Url">
        </div>
      </div>
      <div class="form-group <?= $e_orden; ?>">
        <label class="col-lg-4 control-label" for="submenu_ord">Orden</label>
        <div class="col-lg-2">
          <input type="number" class="form-control" name="submenu_ord" id="submenu_ord" value="<?= obt_valor('submenu_ord',$submenu_ord); ?>" >
        </div>
      </div>
