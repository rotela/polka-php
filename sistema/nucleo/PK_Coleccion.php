...
* Se puede usar en las librerías del usuario con la función otb_instancia(), ejemplo:
...
class PK_Coleccion
{
	...
	{
		if (empty($libreria)) {
			exit(mostrar_error(__CLASS__,'Nombre de librería vacía'));
		}
	...
	private static function verificar($libreria='')
	{
		...
		{
			exit(mostrar_error('PK_Coleccion',"Debes indicar el espacio de nombre completo de <strong>$libreria</strong>"));
		}
		else
		{
			...
			{
				exit(mostrar_error('PK_Coleccion',"No existe la librería o modelo <strong>$archivo</strong>"));
			}
		}
	}
}
