            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                if (count($_POST) > 0) {
                    if (!isset($_POST[$csrf_nom])) {
                        $this->agr_error($csrf_nom, 'Petición corrupta, no existe el campo '.$csrf_nom);
                    } else {
                        if (!isset($_SESSION[$csrf_nom])) {
                            $this->agr_error($csrf_nom, 'Petición corrupta, no existe la sesión '.$csrf_nom);
                        } else {
