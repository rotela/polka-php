...
class numero_letra
{
    ...
    public function to_word($number, $miMoneda = null)
    {
        if (strpos($number, $this->decimal_mark) === false) {
            ...
            $this->convertir_numero($number, $miMoneda, 'entero'),
          );
        } else {
            $number = explode($this->decimal_mark, str_replace($this->separator, '', trim($number)));

            $convertedNumber = array(
            $this->convertir_numero($number[0], $miMoneda, 'entero'),
            $this->convertir_numero($number[1], $miMoneda, 'decimal'),
          );
        }
     ...
     */
    private function convertir_numero($number, $miMoneda)
    {
        $converted = '';
        if ($miMoneda !== null) {
            try {
