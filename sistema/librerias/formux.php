...
class formux{
    ...
    {
        if($apertura) $this->formulario($frNombre='MiFormulario',$frAccion='#',$frMetodo='POST');
    }
    ...
    function agr_campo($camNombre='MiCampo',$camId='',$valor='',$tipo='text',$size=50)
    {
        ...
        if($this->saltoEntrada) $salto = $this->saltoString;
        if(empty($camId))
        {
    ...
    {
        if( ! $this->sinCierreForm) $this->lineas[]='</form>'."\n";//última linea de codigo del form
